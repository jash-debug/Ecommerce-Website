...
export default function Customers() {
  ...
  const paginationItems = Array.from({ length: totalPages }, (_, index) => index + 1)
    ...
    .reduce((acc, currentPage, index, source) => {
      if (index > 0 && currentPage - source[index - 1] > 1) {
        acc.push("...");
      }
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div className="space-y-2">
            ...
            <p className="max-w-2xl text-sm text-muted-foreground">
              Manage admins, moderators, and users from the DummyJSON API.
            </p>
          </div>
                ...
                <table className="min-w-full divide-y divide-border/70 text-left text-sm">
                  ...
                  <tbody className="divide-y divide-border/70">
                    {users.length === 0 ? (
                      ...
                      users.map((user) => (
                        <tr key={user.id} className="align-top transition hover:bg-muted/30">
                          ...
                          <td className="px-4 py-4 text-muted-foreground">@{user.username}</td>
                          <td className="px-4 py-4 text-muted-foreground">{user.email}</td>
                          <td className="px-4 py-4">{roleBadge(user.role)}</td>
                          <td className="px-4 py-4 text-muted-foreground">{user.age}</td>
